...
| **2. Diagnose** | The `RCA Agent` (LangChain ReAct + Claude Sonnet 5 via the Requesty AI gateway) investigates each event group using read-only MCP tools (`kubectl_get`, `kubectl_describe`, `kubectl_logs`). It writes a structured Markdown report with root cause, severity, affected resources, and suggested fixes. |
...
- Both agents use **LangChain `create_agent`** (ReAct pattern) with **Claude Sonnet 5** via the **Requesty AI gateway**
...
| **Claude Sonnet 5** | LLM via the Requesty AI gateway (`langchain-openai`, OpenAI-compatible) |
...
└── src/my_agent_app/
    ...
    │
    ├── agents/
    │   ├── llm.py                 # ChatOpenAI factory (Requesty AI gateway)
...
cp .env.example .env
# Edit .env — set REQUESTY_API_KEY and BASIC_AUTH_PASSWORD
```
